...
async def dispatch_call(lk_client: api.LiveKitAPI, payload: dict):
    ...
    try:
        ...
        logger.error(
            f"[Call {call_id}] Agent dispatch failed: {e}\n{traceback.format_exc()}"
        )
        raise e

    try:
        logger.info(
            f"[Call {call_id}] Initiating SIP call to {phone_number} via trunk {trunk_id}"
        )
        ...
        logger.info(
            f"[Call {call_id}] SIP Participant created: {sip_part.participant_identity}"
        )
    except Exception as e:
        logger.error(
            f"[Call {call_id}] SIP Call trigger failed: {e}\n{traceback.format_exc()}"
        )
        raise e
    ...
    try:
        ...
        for call_id, room_name in active_calls.items():
            if room_name not in active_rooms:
                logger.warning(
                    f"Zombie detected! Room {room_name} not found in LiveKit. Removing call {call_id} from active."
                )
                await redis_client.hdel("calls:active", call_id)
                await redis_client.set(
                    f"calls:status:{call_id}", "completed_or_failed_zombie"
                )
    except Exception as e:
...
async def main():
    ...
    logger.info(
        f"Dispatcher started. Limits: MaxConcurrency={MAX_CONCURRENCY}, Agent={AGENT_MAX_WORKERS}, LiveKit={LIVEKIT_MAX_ROOMS}"
    )
                ...
                if available_capacity > 0:
                    ...
                    if popped:
                        ...
                        room_name = payload.get(
                            "_resolved_room_name", f"call_{call_id}"
                        )

                        logger.info(
                            f"Dequeued call {call_id}. Capacity before: {active_count}. Available: {available_capacity}"
                        )
                        ...
                        try:
                            ...
                            await redis_client.set(
                                f"calls:status:{call_id}", "in_progress"
                            )
                        except Exception:
                            # Re-queue on failure and free up capacity
                            logger.error(
                                f"Failed to dispatch {call_id}. Re-queueing..."
                            )
                            await redis_client.hdel("calls:active", call_id)
                            # Increment score to push it back slightly, or keep same score
                            await redis_client.zadd(
                                "queue:pending", {call_entry: score + 10}
                            )
                            await redis_client.set(
                                f"calls:status:{call_id}", "failed_dispatch_requeued"
                            )
            except Exception as e:
