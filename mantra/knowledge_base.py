...
class KnowledgeBase(ABC):
    ...
    @abstractmethod
    async def search(self, kb_id: str, query_embedding: list[float], top_k: int = 3, threshold: float = 0.7) -> list[KnowledgePage]:
...
class PostgresKnowledgeBase(KnowledgeBase):
    ...
    async def delete_page(self, page_id: str) -> bool:
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            result = await conn.execute("DELETE FROM kb_pages WHERE id = $1", uuid.UUID(page_id))
...
def detect_structure(text: str) -> str:
    ...
    lines = text.split('\n')
    heading_count = sum(1 for l in lines if l.strip().startswith(('#', '##', '###', 'Section', 'SECTION', 'Chapter', 'CHAPTER')))
...
def chunk_by_heading(text: str, max_tokens: int = 2000) -> list[dict]:
    ...
    for line in text.split('\n'):
        ...
        is_heading = line_stripped.startswith(('#', '##', '###', 'Section', 'SECTION', 'Chapter', 'CHAPTER'))
...
def chunk_by_sliding_window(text: str, max_tokens: int = 2000, overlap: int = 200) -> list[dict]:
...
async def ingest_file(kb: KnowledgeBase, kb_id: str, file_bytes: bytes, filename: str) -> dict:
